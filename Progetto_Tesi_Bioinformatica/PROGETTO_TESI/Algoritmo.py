...
if __name__ == "__main__":
    cromosomi = ["cromosoma20"]
    #for i in range(24):
    #    cromosomi.append("cromosoma"+str(i+1))
    nuoveFinestreCalcolate = True
    ...
    for nomefile in cromosomi:
        ...
        FunzioniBio.gestisci_cartelle(cartellaRisultati)
        if abilitaFileDebug:
        ...
        if(not nuoveFinestreCalcolate):
            ...
            pos = []
            conta_finestre = []
            FunzioniBio.ricalcoloFinestre(cartellaRisultati, nomefile, cromosoma, pos, conta_finestre, finestra, finestre_mers, matches_finestra, DIVISORE_GRAFICO_RIPETIZIONI, PERCENTUALE, NUM_BP_FINESTRA)
        ...
        if(not distanzeCalcolate):
            FunzioniBio.calcoloDistanzeMatch(cartellaRisultati, nomefile, abilitaFileDebug, ftt, NUM_BASI_MER, NUMERO_CORE)
