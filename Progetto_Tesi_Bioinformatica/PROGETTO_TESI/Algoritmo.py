...
cont = 0
presente = False
while cont < len(distanze):
    conta_distanze = 0
    presente = False
    j = cont - 1
    while j >= 0:
        if distanze[cont] == distanze[j]:
            presente = True
            break
        else:
            j = j - 1
    if presente:
        ...
        while i < len(distanze):
            if distanze[cont] == distanze[i]:
                conta_distanze = conta_distanze + 1
            i = i + 1
        freq_distanze.append(conta_distanze)
        cont = cont + 1
...
while cont < len(distanze):
    ...
    if presente:
        ...
        n_distanze.append(distanze[cont])
        cont = cont + 1

#creazione istogramma similarita' media di ciscuna finestra
figura, grafico = plt.subplots()
grafico.bar(x=n_distanze, height=freq_distanze, fc="orange")
...
distanza_frequente = n_distanze[freq_distanze.index(max(freq_distanze))]
