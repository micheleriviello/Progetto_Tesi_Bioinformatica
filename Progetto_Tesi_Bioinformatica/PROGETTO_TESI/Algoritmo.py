...
def gestisci_buffer(buffer, executor, diz_dis_freq):
    if(len(buffer) == NUMERO_CORE):
        ...
        for risultato in risultati:
            for chiave in risultato.keys():
                if chiave not in diz_dis_freq:
                    diz_dis_freq[chiave] = risultato[chiave]
                else:
                    diz_dis_freq[chiave] = diz_dis_freq[chiave] + risultato[chiave]
            print("FINE RISULTATO")
        buffer.clear()

def svuota_buffer(buffer, executor, diz_dis_freq):
    if(len(buffer) > 0):
        results = [executor.submit(calcola_match, k) for k in buffer]
...
if __name__ == "__main__":
    ...
    for nomefile in cromosomi:
                        ...
                        for j in range(i, len(listaRipetizioni)):
                            if listaRipetizioni[j] > listaRipetizioni[i] + NUM_BASI_MER:
                                ...
                                if distanza in diz_dis_freq:
                                    ...
                                    diz_dis_freq[distanza] = 1
        for chiave in buffers:
            svuota_buffer(buffers[chiave], executor, diz_dis_freq)
        end = time.time()
        ...
        with open(cartellaRisultati+"distanza_frequenze"+nomefile, "w") as fl:
            json.dump(diz_dis_freq, fl)

        with open(cartellaRisultati+"distanza_frequenze"+nomefile, "r") as fl:
