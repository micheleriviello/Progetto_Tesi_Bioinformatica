...
----------------------------------------------------------------------
'''
def ricalcoloFinestre(cartellaRisultati, nomefile, cromosoma, pos, conta_finestre, finestra, finestre_mers, matches_finestra, DIVISORE_GRAFICO_RIPETIZIONI, PERCENTUALE, NUM_BP_FINESTRA):
...
--------------------------------------------------------------------------
'''
def calcoloDistanzeMatch(cartellaRisultati, nomefile, abilitaFileDebug, ftt, NUM_BASI_MER, NUMERO_CORE):
    ...
    writer.writerow(["Posizione iniziale", "Posizione finale", "Dimensione", "Entropia Shannon", "Numero ripetizioni"])
    ...
    for cont in range(0, len(nuove_finestre)):
        ...
        writer.writerow([datiFinestra.posIniziale, datiFinestra.posFinale, datiFinestra.numeroBasi, datiFinestra.entropiaShannon, datiFinestra.numRipetizioni])
...
def gestisci_cartelle(cartellaRisultati):
...
----------------------------------------------------------------------------------------------
'''
def mostra_alte_ripetizioni(cromosoma, precisione):
    ...
    grafico.set_title("Zone ad alta ripetizione (da 1000 finestre non contigue)")
    figura.set_figwidth(9)
    figura.set_figheight(2)
    grafico.set_ylim([0, 1])
    figura.gca().axes.get_yaxis().set_visible(False)
    grafico.bar(x=np.arange(len(visualizzatore_cromosoma)), height=visualizzatore_cromosoma, fc="orange", align="edge", width=0.8, label="= 1000 finestre")
...
----------------------------------------------------------------------------------------------
'''
def mostra_istogramma(cromosoma, numeroFinestreStampate, entropiaMax, entropiaMin, posInizialeMin):
    cartellaRisultati = "risultati/"+cromosoma+"/"
    ...
    for finestra in finestre:
        ...
        if condizione < entropiaMax and condizione > entropiaMin and numeroFinestra < numeroFinestreStampate and finestra["posIniziale"] > posInizialeMin:
