...
def mostra_alte_ripetizioni(cromosoma, precisione):
    ...
    grafico.set_title("Zone ad alta ripetizione (da 1000000 di basi azotate)")
    figura.set_figwidth(9)
    figura.set_figheight(2)
    grafico.set_ylim([0, 1])
    figura.gca().axes.get_yaxis().set_visible(False)
    grafico.bar(x=np.arange(len(visualizzatore_cromosoma)), height=visualizzatore_cromosoma, fc="orange", align="edge", width=0.8, label="= 1000000 di basi azotate")
