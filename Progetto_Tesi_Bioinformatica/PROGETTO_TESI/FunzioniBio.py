...
def calcoloDistanzeMatch(cartellaRisultati, nomefile, abilitaFileDebug, ftt, NUM_BASI_MER, NUMERO_CORE):
    ...
    for cont in range(0, len(nuove_finestre)):
        ...
        writer.writerow([datiFinestra.posIniziale, datiFinestra.posFinale, datiFinestra.numeroBasi, datiFinestra.entropiaShannon, datiFinestra.numeroBasi])
...
def mostra_istogramma(cromosoma, numeroFinestreStampate, entropiaMax, entropiaMin, posInizialeMin):
    ...
    for finestra in finestre:
        if finestra["entropiaShannon"] < entropiaMax and finestra["entropiaShannon"] > entropiaMin and numeroFinestra < numeroFinestreStampate and finestra["posIniziale"] > posInizialeMin:
