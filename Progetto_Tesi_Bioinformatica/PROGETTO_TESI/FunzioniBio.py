...
def calcoloDistanzeMatch(cartellaRisultati, nomefile, abilitaFileDebug, ftt, NUM_BASI_MER, NUMERO_CORE):
    ...
    for cont in range(0, len(nuove_finestre)):
        if len(nuove_finestre[cont]) >= 20000:
            diz_dis_freq = {}
            diz = {}
            stringaStampa = ""
            ...
            dizDistanze["_I_"+str(dimFinestre[cont])+"_F_"+str(dimFinestre[cont]+len(nuove_finestre[cont]))] = diz_dis_freq
            print("_I_"+str(dimFinestre[cont])+"_F_"+str(dimFinestre[cont]+len(nuove_finestre[cont])))

    with open(cartellaRisultati+"SavedFiles/Distanze/distanze_"+nomefile+".json", "w") as fl:
        json.dump(dizDistanze, fl)
    #distTemp.close()
...
----------------------------------------------------------------------------------------------
'''
def mostra_istogramma(cromosoma):
    cartellaRisultati = "risultati/"+cromosoma+"/"
    ...
    numeroFinestra = 0
    for chiave in distanze.keys():

        diz_dis_freq = distanze[chiave]
        Istogrammatore(diz_dis_freq, chiave)
        '''
